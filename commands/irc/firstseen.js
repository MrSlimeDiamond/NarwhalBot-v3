...
class LastseenCommand extends IRCCommand {
    constructor(irc) {
        ...
        this.name = 'firstseen'
        this.aliases = ['fs', 'firstjoin']
        this.description = "Get a player's first join date"
    }
    ...
    async onCommand(ircclient, discordclient, from, to, message) {
        ...
        if (body.error == 'Could not find player') {
            ...
            const { firstseen, username } = body
            if (firstseen == 'NOTFOUND') {
                this.sendIRCMessage(
                    to,
            ...
            const date = new Date(parseInt(firstseen) * 1000)
            const now = new Date()
            this.sendIRCMessage(
                to,
                `${username} first logged in at ${date.toUTCString()} (${di.format(
                    date,
                    now
                )} ago)`,
                this.isHidden(message)
